...
public class ThiefDefenderStore {
    ...
    public long insertOrIgnore(ContentValues values) {
        ...
        try {
            retID = db.insertWithOnConflict(TABLE_NAME, null, values, SQLiteDatabase.CONFLICT_IGNORE);
        } catch (Exception e) {
    ...
    class DbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            String sql = "create table " + TABLE_NAME + " (" + C_ID
                    + " int primary key, " + C_START_TIME + " text, " + C_END_TIME
                    + " text, " + C_DURATION + " text, " + C_EVENT + " text)";
            db.execSQL(sql);
